...
DEFAULT_ROUTE=192.168.1.0/24 192.168.1.254

nmcli con add ifname "${BRIDGE}" type bridge con-name "${BRIDGE}"
nmcli con modify "${BRIDGE}" bridge.stp "${BRIDGE_STP}"
nmcli con modify "${BRIDGE}" ipv4.method manual ipv4.address "${IP_ADDRESS}" ipv4.dns "${DNS_SERVERS}" ipv4.routes "${DEFAULT_ROUTE}" ipv6.method disable
nmcli con up "${BRIDGE}"
